    ...
    if (req.body && req.method === 'POST' && req.body.SAMLResponse) {
      ...
      var loaded = function (err, ok, state) {
        ...
        self._samlp.validateSamlResponse(samlResponseDom, function (err, profile) {
          if (err) {
            self._emitSamlpResponseInfoEvent(samlResponseDom, false);
            return self.fail(err, err.status || 400)
          };

          var verified = function (err, user, info) {
            self._emitSamlpResponseInfoEvent(samlResponseDom, !err && user);
...
WsFedSaml2Strategy.prototype._emitSamlpResponseInfoEvent = function(dom, success) {
  function getAttribute(ns, tag, attribute) {
    var node = dom.getElementsByTagNameNS(ns, tag)[0];
    return node && node.getAttribute(attribute);
  }
  function getValue(ns, tag) {
    var node = dom.getElementsByTagNameNS(ns, tag)[0];
  ...
  try {
    ...
    var authnInstant = getAttribute(ns, 'AuthnStatement', 'AuthnInstant');
    var issueInstant = getAttribute(ns, 'Assertion', 'IssueInstant');
    var contextClassRef = getValue(ns, 'AuthnContextClassRef');
    var audience = getValue(ns, 'Audience');

    var conditions = {};
    var conditionsNode = dom.getElementsByTagNameNS(ns, 'Conditions')[0];
    if(conditionsNode && conditionsNode.attributes) {
      for(var i = 0; i < conditionsNode.attributes.length; ++i) {
        var attr = conditionsNode.attributes[i];
        conditions[attr.name] = attr.value;
      }
    }

    var attributes = {};
    var attributesNodeList = dom.getElementsByTagNameNS(ns, 'Attribute');
    for(var i = 0; i < attributesNodeList.length; ++i) {
      var attr = attributesNodeList[i];
      var name = attr.getAttribute('Name');
      var value = attr.firstChild.textContent;
    ...
    var responseInfo = {
      ...
      contextClassRef: contextClassRef,
      audience: audience,
      conditions: conditions,
      attributes: attributes
    };

    this.events.emit('samlpResponseInfo', responseInfo);
  } catch(e) {
    // This code 👆 is very error prone. This is a safety net so that logins
    // don't fail for something as dumb as this.
    this.events.emit('samlpResponseInfo', {
      error: e.toString()
    });
  }
}
